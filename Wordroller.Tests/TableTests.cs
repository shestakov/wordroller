...
namespace Wordroller.Tests
{
	public class TableTests: TestsBase
	{
		...
		public void VerticalMerge()
		{
			...
			var table = section.AppendTable(new CreateTableParameters(WidthUnit.Pc, 100, 3, WidthUnit.Pc, new double[] { 33f, 33f, 33f }, DefaultTableStyles.TableNormal));
		...
		public void Table_Width_Fixed()
		{
			...
			var table = section.AppendTable(new CreateTableParameters(WidthUnit.Cm, 8f, 3, WidthUnit.Cm, new double[] { 1f, 1f, 2f }, DefaultTableStyles.TableNormal));
			var row = table.Rows.First();
			var cells = row.Cells.ToArray();
